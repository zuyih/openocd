...
inline enum tas_device_family_t tas_get_device_family(uint32_t device_type) {
...
//! \returns \c true if yes, otherwise \c false
inline bool tas_device_family_is_aurix(enum tas_device_family_t device_family) {
  return ((device_family == TAS_DF_TC4X) || (device_family == TAS_DF_TC3X) ||
          (device_family == TAS_DF_TC2X));
}

//! \brief Performs a check if device type falls into an AURIX family
//! \returns \c true if yes, otherwise \c false
inline bool tas_device_type_is_aurix(uint32_t device_type) {
...
//! family
inline const char *
tas_get_device_family_str(enum tas_device_family_t device_family) {
