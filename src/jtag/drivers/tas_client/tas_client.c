...
struct tas_client_state {
  int sock;
  bool connected;
  /* TAS server, from "tas-client host"; NULL for the default. */
  char *host;
  char *port;
  /* Access hardware clock from "adapter speed", 0 while none was given. */
  int speed_khz;
...
static int tas_client_init(void) {
  const char *host = client_state.host ? client_state.host : "localhost";
  const char *port = client_state.port ? client_state.port : "24817";
  ...
  if (err) {
    LOG_ERROR("Cannot resolve TAS server %s: %s", host, gai_strerror(err));
    return ERROR_FAIL;
  }
  ...
  if (client_state.sock == -1) {
    LOG_ERROR("Failed to connect to TAS server %s:%s: %s", host, port,
              strerror(errno));
...
static int tas_client_quit(void) {
  ...
  free(client_state.host);
  client_state.host = NULL;
  free(client_state.port);
  client_state.port = NULL;
...
/* Access hardware clock in Hz, TAS_AM15_RW_ACC_HW_FREQUENCY of the TAS client
 * API. A frequency the hardware cannot do is rounded to one it can. */
#define TAS_AM15_ACC_HW_FREQUENCY 0x000C0100

static int tas_client_apply_speed(uint8_t con_id) {
  int err = tas_client_write_am15_u32(client_state.sock, con_id,
                                      TAS_AM15_ACC_HW_FREQUENCY,
                                      client_state.speed_khz * 1000);
  if (err)
    LOG_ERROR("Failed to set the access hardware to %d kHz",
              client_state.speed_khz);
...
static int tas_client_op_connect(struct aurix_ocds *ocds) {
   ...
   * that is ambiguous, so "adapter serial" may name one by the identifier the
   * server reports for it.
   */
  for (i = 0; i < client_state.target_num; i++) {
    if (serial && strcmp(client_state.targets[i].identifier, serial))
      continue;
    for (j = 0; j < ocds->tap->expected_ids_cnt; j++) {
       ...
       * several OCDS spread over the attached devices. */
  ...
  if (!target && matches > 0) {
    LOG_ERROR("OCDS %s: every matching device is already claimed by another "
              "OCDS. One OCDS covers all cores of a device.",
              ocds->name);
  ...
  if (!target) {
    if (serial)
      LOG_ERROR("OCDS %s: no attached device is \"%s\" (adapter serial) with "
                "one of the %u expected TAP ID(s) of %s",
                ocds->name, serial, ocds->tap->expected_ids_cnt,
  ...
  if (matches > 1) {
    LOG_WARNING("OCDS %s: %u attached devices match, using \"%s\"; pick one "
                "with: adapter serial \"<identifier>\"",
                ocds->name, matches, target->identifier);
    for (i = 0; i < client_state.target_num; i++)
  ...
  if (client_state.speed_khz) {
    err = tas_client_apply_speed(ocds->con_id);
    if (err)
      return err;
  }
...
/*
 * The access hardware clock is a setting of a TAS session, and a session is
 * only started once an OCDS connects; one started later picks the clock up
 * there.
 */
static int tas_client_speed(int speed) {
  client_state.speed_khz = speed;

  for (size_t i = 0; i < ARRAY_SIZE(client_state.con_queues); i++) {
    if (!client_state.con_queues[i].reqs)
      continue;
    int err = tas_client_apply_speed(i);
    if (err)
      return err;
  }
...
COMMAND_HANDLER(tas_client_handle_host_command) {
  ...
  free(client_state.host);
  free(client_state.port);
  client_state.host = strdup(CMD_ARGV[0]);
  client_state.port = CMD_ARGC == 2 ? strdup(CMD_ARGV[1]) : NULL;
  if (!client_state.host || (CMD_ARGC == 2 && !client_state.port)) {
...
static const struct command_registration tas_client_subcommand_handlers[] = {
    {
        ...
        .mode = COMMAND_CONFIG,
        .help = "Set the TAS server to connect to, by name or address "
                "(default localhost, port 24817).",
        .usage = "host [port]",
    },
