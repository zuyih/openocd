...
enum : uint8_t {
	...
	TAS_AM1   = 1,	/*!< \brief Auxiliary address map for device access. E.g. direct EEC access
			 * of AURIX EDs */
...
enum {
	TAS_AM15_JTAG_MAX_NUM_SCAN_BITS_PER_TRANSACTION = 1024*8/*!< \brief Maximum number of bit
								 * per transaction */
};
...
enum tas_am15_addr_et : uint32_t {
	...
	/*! \brief Read or write the frequency [Hz] of the device access HW clock (JTAG, DAP, SWD,
	 * etc.). */
	/*! \details When the value is written the TasServer chooses the closest possible lower
	...
	/*! \brief Select the scan chain with the JTAG instruction register by writing
	 * tas_jtag_set_ir_st */
	TAS_AM15_W_JTAG_SET_IR  = 0x000C1300,

	/*! \brief Capture selected JTAG scan chain */
	/*! \details The uint32_t num_scan_bits value written is the number of scan chain bits
	 * shifted */
	...
	/*! \brief Time of the TasServer start in micro seconds elapsed since midnight, January 1,
	 * 1970 */
...
enum tas_am15_userpins_cntrl_et : uint32_t {
	...
	TAS_UPC_ADD_SFP_TIP   = 2,/* TAS_UP_ALL_UP, TAS_UP_SFP_RESET, TAS_UP_SFP_JTAG or
				 * TAS_UP_SFP_DAP */
};
...
enum tas_am15_userpins_et : uint32_t {
	...
	TAS_UP_USR0         = 0x00000001,/*!< \brief DAP connector: USR0 bidi; OCDS L1 connector:
					 * OCDSE t2d */
	...
	TAS_UP_RUNLED       = 0x00008000,/*!< \brief Target is running LED on miniWiggler and
					 * on-board. Low active. */


	TAS_UP_ALL_SFP      = 0xFFFF0000,/*!< \brief All potential special function pins of an
					 * access HW */
	...
	TAS_UP_SFP_ALL_JTAG = 0xF0000000,	/*!< \brief All controllable JTAG pins (TRST, TDI,
						 * TMS, TCK) t2d */
	TAS_UP_SFP_TDI      = 0x20000000,/*!< \brief JTAG TDI */
	TAS_UP_SFP_TMS      = 0x40000000,/*!< \brief JTAG TMS */
	TAS_UP_SFP_TCK      = 0x80000000,/*!< \brief JTAG TCK */

	/* Additional special function pins if TAS_UPC_ADD_SFP_TIP and (TAS_DEV_CON_PHYS_DAP or
	 * TAS_DEV_CON_PHYS_SWD) */
